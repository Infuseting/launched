...
pub async fn install_forge(mc_version: &str, forge_version: &str) -> Result<(), String> {
    ...
    if !version_dir.exists() {
        tokio_fs::create_dir_all(&version_dir).await.map_err(|e| format!("Failed to create Forge version directory: {}", e))?;
    }
    ...
    if !installer_path.exists() {
        ...
        let bytes = response.bytes().await.map_err(|e| format!("Failed to read Forge installer: {}", e))?;
        tokio_fs::write(&installer_path, &bytes).await.map_err(|e| format!("Failed to save Forge installer: {}", e))?;
    }
...

    {
        ...
        lib_dir = mc_path.join("libraries")
            .join("net")
            .join("minecraftforge")
            .join("forge")
            .join(format!("{}-{}", mc_version, forge_version));
        
        fs::create_dir_all(&lib_dir).map_err(|e| format!("Failed to create forge lib dir: {}", e))?;
        dest_jar = lib_dir.join(format!("forge-{}-{}.jar", mc_version, forge_version));

        // 2.2 Extract universal jar
        let mut universal_jar = archive.by_name(&universal_name)
            .map_err(|e| format!("Universal jar {} not found in installer: {}", universal_name, e))?;
    ...
    if let serde_json::Value::Object(ref mut map) = version_info_obj {
        map.insert("id".to_string(), serde_json::Value::String(forge_id.clone()));
    }
    ...
    let manifest: crate::core::launch::models::VersionManifest = serde_json::from_str(&version_json_str)
        .map_err(|e| format!("Failed to parse final forge JSON for library downloads: {}", e))?;
...
pub async fn is_forge_installed(mc_version: &str, forge_version: &str) -> bool {
    if let Ok(mc_path) = get_official_mc_path().await {
        let forge_id = format!("{}-forge-{}", mc_version, forge_version);
        mc_path.join("versions").join(&forge_id).join(format!("{}.json", forge_id)).exists()
    } else {
        false
    }
}

...
pub async fn install_fabric(mc_version: &str, loader_version: &str) -> Result<(), String> {
    ...
    if !version_dir.exists() {
        fs::create_dir_all(&version_dir).await.map_err(|e| format!("Failed to create Fabric version directory: {}", e))?;
    }
    ...
    for lib in profile.libraries {
        ...
        let lib_path = mc_path.join("libraries").join(package).join(name).join(version).join(&jar_name);
        
        if !lib_path.exists() {
            if let Some(parent) = lib_path.parent() {
                fs::create_dir_all(parent).await.map_err(|e| format!("Failed to create library directory: {}", e))?;
            }
...
fn package_path_to_maven_path(name: &str) -> String {
    ...
    format!("{}/{}/{}/{}-{}.jar", package, name_str, version, name_str, version)
}
